...
if [ -z "${DOCKER_PKG_DOCKERFILES:-}" ] && [ -x ~/docker/dockerfiles/.libs/git.sh ]; then
	DOCKER_PKG_DOCKERFILES=~/docker/dockerfiles
fi
[ -d "$DOCKER_PKG_DOCKERFILES" ]
source "$DOCKER_PKG_DOCKERFILES/.libs/git.sh"
...
hook_sha256() {
	...
	local json
	json="$(jq <<<"$sha256" -csR --arg urlBase "$urlBase" -L"$DOCKER_PKG_DOCKERFILES/.libs" '
		include "lib"
		;
		split("\n")
		| map(
